...
docker run --rm -it \
    -v $(pwd):/workspace \
    registry.cn-hangzhou.aliyuncs.com/aicpn/centos-compiler:7.9.2009-amd64 bash -c "
    ...
    # 配置编译环境
    cmake -G Ninja -DCMAKE_C_COMPILER=/usr/local/bin/gcc -DCMAKE_CXX_COMPILER=/usr/local/bin/g++ -DCRANE_USE_SYSTEM_LIBCGROUP=ON ..
