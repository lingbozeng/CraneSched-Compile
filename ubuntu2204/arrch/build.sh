...
docker run --rm -i \
    -v $(pwd):/workspace \
    -e CC='/usr/local/gcc-14.2.0/bin/gcc-14' \
    -e CXX='/usr/local/gcc-14.2.0/bin/g++-14' \
    registry.cn-shanghai.aliyuncs.com/pkuscow/ubuntu2204_arrch64:crane \
    bash -c "
    # 在容器内输出环境变量以供调试
    echo \"CC inside container is: \$CC\"
    echo \"CXX inside container is: \$CXX\"

    # 克隆项目并切换到指定分支
    cd /workspace
    git clone https://gitee.com/pku_icode/CraneSched.git
    cd CraneSched
